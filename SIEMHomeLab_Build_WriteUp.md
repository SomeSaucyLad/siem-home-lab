...
## Build Steps
1. Downloaded the latest Security Onion distribution and verified the SHA256 hash against the official Security Onion Solutions website.
2. Followed the official installation guide from the Security Onion Solutions website and configured the VM to the recommended hardware/network settings.
   
![VMWare Settings](/images/01-VMWare_Settings.png)
4. After VM configuration was complete, I began the setup process, choosing the Evaluation installation configuration due to the lab's small size.

![Evaluation Installaion Selection](/images/02-Installation_Mode.png)
5. I then configured my network interfaces, chosing ens160 as the NIC used for the management interface and ens192 for the monitor interface.

![NIC Configuration](/images/03-NIC_Config.png)
6. Configured the management interface to use a static IP, verifying there were no other hosts on that IP range, and added credentials to access the web portal later.

![Setup Overview](/images/04-Setup_Overview.png)
7. After completing the setup process, I signed in to the web portal via the IP address I configured earlier.
